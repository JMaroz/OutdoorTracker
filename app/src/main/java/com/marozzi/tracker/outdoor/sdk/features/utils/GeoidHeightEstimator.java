...
/**
 * GeoidHeightEstimator uses 1 degree grid data of GeoidHeightEstimator geoid height data stored in
...
public class GeoidHeightEstimator {
    ...
    private static GeoidHeightEstimator ourInstance;

    public static GeoidHeightEstimator getInstance(Context context) {
        if (ourInstance == null || assets == null) {
            ourInstance = new GeoidHeightEstimator(context.getAssets());
        }
        return ourInstance;
    }

    private GeoidHeightEstimator(AssetManager assets) {
     ...
     */
    public int computeGeoidHeight(double latitude, double longitude) {
     ...
     */
    private Short[] getHeightsAt(double latitude, double longitude) {
     ...
     *                     may be left blank
     */
    private void readValues(Short[] results, GeoIndex sw, GeoIndex nw, GeoIndex ne, GeoIndex se)
            throws IOException {
        if (assets == null) {
     ...
     */
    private int readShort(DataInputStream in, int position, GeoIndex index, Short[] store,
